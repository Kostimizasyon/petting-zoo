#include "Missile.h"
#include "Aircraft.h"

Missile::Missile(const GLfloat direction_x, const GLfloat direction_y, const GLfloat speed_x, const GLfloat speed_y, double drag_area, double lift_area, double mass,
	const std::string& nato_designation,
	double engine_power, double engine_time, double sustainer_power, double sustainer_time)
	: AerialObject(direction_x, direction_y, speed_x, speed_y, drag_area, lift_area, mass),
	nato_designation(nato_designation),
	engine_power(engine_power), engine_time(engine_time),
	sustainer_power(sustainer_power), sustainer_time(sustainer_time)
{}

//MY MAIN GOAL IS TO BLOW UP
void Missile::explode() {
	this->~Missile();
}

// Get shot
void Missile::launch(const Aircraft& launch_aircraft, const float dt) {
	// 1. Inherit position (X & Y / Altitude) from the aircraft
	set_direction_x(launch_aircraft.get_direction_x());
	set_direction_y(launch_aircraft.get_direction_y() - 5.0f, false);

	// 2. Inherit initial velocity vector (Speed X & Y) from the aircraft
	set_speed_x(launch_aircraft.get_speed_x(), false);
	set_speed_y(launch_aircraft.get_speed_y(), false);

	recalculate_environment();

	move(dt);
}

// Implementing virtual

double Missile::get_push_force() {
	if (engine_time > 0)			    return engine_power;
	else if (sustainer_power > 0)	    return sustainer_power;
	else							    return 0; 
}

void Missile::decrement_dt(const float dt) {
	if (engine_time > 0)		   engine_time -= dt;
	else if (sustainer_power> 0)   sustainer_time -= dt;
}

void Missile::display_stats() {
		
	std::cout << "--- "
				<< nato_designation 
				<< " TELEMETRY---" << "\n"
				<< "Phase:    "    << (engine_time > 0 ? "BOOST" : sustainer_time > 0 ? "SUSTAIN" : "GLIDE") << "\n"
				<< "Speed X:  "    << std::fixed << std::setprecision(2) << speed_x << " m/s (" << (speed_x * 3.6) << " km/h)\n"
				<< "Speed Y:  "    << speed_y << " m/s\n"
				<< "Mach:     "    << std::setprecision(3) << mach_number << "\n"
				<< "Altitude: "    << (int)altitude << " m\n"
				<< "-------------------------" << "\n";

}
