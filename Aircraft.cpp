#include "Aircraft.h"
	...
	Aircraft::Aircraft(const GLfloat direction_x, const GLfloat direction_y, const GLfloat speed_x, const GLfloat speed_y, double drag_area, double lift_area, double mass,
	...
	void Aircraft::decrement_dt(const float dt) {
		if (is_aft && fuel_load > 0) {
			...
			mass -= wet_usage_sec * dt;
		}
		else if (!is_aft && fuel_load > 0) {
			fuel_load -= dry_usage_sec * dt;
			mass -= dry_usage_sec * dt;
		}
	}
